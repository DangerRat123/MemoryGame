...
let carouselIndex = 0;
let carouselRotationSteps = 0; // unbounded — keeps counting up/down forever, never wraps
...
function step(direction) {
  const len = carouselLocations.length;
  // Just keep counting in this direction forever — this is what makes the
  // drum spin continuously past location 6 back to 1, rather than
  // snapping backward once the index wraps.
...
function goToIndex(targetIdx) {
  ...
  // Take whichever direction is the shorter spin to reach the clicked card.
